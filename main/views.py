...
def add_to_cart(request, slug):
    ...
    cart_item = CartItems.objects.update_or_create(
        item=item,
        user=request.user,
        ordered=False,
    )
...
def pay(request):
    ...
    client = razorpay.Client(auth=("rzp_test_1P933hawOQIB24", "7IOm1J6Xqcm9G4cL9WGUIqg0"))

    data = { "amount": 200, "currency": "GBP", "receipt": "#11" }
    payment = client.order.create(data=data)

    return render(request,'main/pay.html', payment= payment)
